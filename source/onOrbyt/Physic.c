...
//	addBufferPos(spacecraft->pos, new_pos, NB_POS);
	...
	for(i=0;i<NB_PLANETS;i++)
	{
		if(planets[i].mu != 0)
		{
			...
			//printf("%lu, %d, %d \n", r2, delta.x, delta.y);
			acc.x+=(planets[i].mu*delta.x)/(r2*r/PHYS_CONST);   // r2*r max = 38'000'000 et mu*delta.x max = 15'360
			acc.y+=(planets[i].mu*delta.y)/(r2*r/PHYS_CONST);	// r2*r max = 38'000'000 et mu*delta.y max = 11'520
		}
	}
	tmp_acc.x+=acc.x;
	tmp_acc.y+=acc.y;

	if(tmp_acc.x>=PHYS_CONST)
	{
		acc.x=tmp_acc.x/PHYS_CONST;
		tmp_acc.x=tmp_acc.x%PHYS_CONST;
	}
	else acc.x=0;

	if(tmp_acc.y>=PHYS_CONST)
	{
		acc.y=tmp_acc.y/PHYS_CONST;
		tmp_acc.y=tmp_acc.y%PHYS_CONST;
	}
	else acc.y=0;
	//printf("%d %d \n", acc.x, acc.y);
